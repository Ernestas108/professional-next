...
export default function Page() {
    ...
    // KREPŠELIS: funkcijos + skaitliukas
    const { add, open, count } = useCart();
    const handleAdd = (p: Product) => {
        add({ id: p.id, title: p.title, price: p.price, img: p.img }); // EUR bazė
        open();
    };
        ...
        <div className="min-h-screen bg-slate-50 text-slate-900">
            ...
            <header className="sticky top-0 z-50 bg-white/85 backdrop-blur border-b border-slate-200">
                <div className="mx-auto max-w-7xl px-4 py-3 flex items-center gap-3">
                    ...
                    {/* 🛒 KREPŠELIO MYGTUKAS su kiekiu (dešinėje) */}
                    <button
                        onClick={open}
                        className="ml-auto relative px-3 py-2 rounded-md border border-slate-300 bg-white hover:bg-slate-100"
                        title="Atidaryti krepšelį"
                    >
                        🛒 Krepšelis
                        {!!count && (
                            <span className="absolute -top-2 -right-2 min-w-[20px] h-[20px] px-1 rounded-full bg-blue-600 text-white text-xs flex items-center justify-center">
                {count}
              </span>
                        )}
                    </button>
                        ...
                        <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            {pageItems.map((p) => (
                                <div key={p.id} className="rounded-xl border border-slate-200 bg-white overflow-hidden">
                                    {/* ⬇️ Paspaudus ant nuotraukos – į krepšelį + atidaryti stalčių */}
                                    <button
                                        onClick={() => handleAdd(p)}
                                        className="block w-full"
                                        aria-label={`Į krepšelį: ${p.title}`}
                                        title="Į krepšelį"
                                    >
                                        <img
                                            src={p.img}
                                            alt={p.title}
                                            className="w-full h-44 object-cover transition-transform duration-200 hover:scale-[1.02] cursor-pointer"
                                            loading="lazy"
                                        />
                                    </button>
